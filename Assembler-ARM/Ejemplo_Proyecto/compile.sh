...
if [ -z "$1" ]; then
    echo File Assembled: $file - $(date +%H:%M:%S)

elif [ "execute" == "$1" ]; then
    echo Executing the file: $file - $(date +%H:%M:%S)
    qemu-aarch64 ./$file

elif [ "debug" == "$1" ]; then
    echo Debugging the file: $file - $(date +%H:%M:%S)
    qemu-aarch64 -g 1234 ./$file &
    gnome-terminal --window --maximize -- bash -c "gdb-multiarch -q --nh -ex 'set architecture aarch64' -ex 'file $file' -ex 'target remote localhost:1234' -ex 'layout split' -ex 'layout regs';"
